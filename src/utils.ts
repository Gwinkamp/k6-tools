...
export function randomItem<T>(arrayOfItems: Array<T>): T {
  return arrayOfItems[Math.floor(Math.random() * arrayOfItems.length)];
}

export function randomString(length: number, charset: string = "abcdefghijklmnopqrstuvwxyz") {
  let res = "";
  while (length--) res += charset[(Math.random() * charset.length) | 0];
  return res;
}

export function findBetween(content: string, left: string, right: string, repeat: boolean = false) {
  const extracted = [];
  const doSearch = true;
  let start,
    end = 0;
...
export function normalDistributionStages(maxVus: number, durationSeconds: number, numberOfStages: number = 10) {
  ...
  const durations = new Array(numberOfStages + 2).fill(Math.ceil(durationSeconds / 6));
  const k6stages = [];
